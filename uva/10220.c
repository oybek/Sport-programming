...
void str(bigint *, const char *); /* O(n) */
...
int digsum( const bigint * const a )
{
	int i, s = 0;
	for ( i = 0; i < a->dign; ++i ) {
		BI_BASE_TYPE d = a->dig[i];
		while ( d > 0 ) {
			s += d%10;
			d /= 10;
		}
...
int main()
{
	...
	while ( scanf("%d", &n) != EOF )
	{
		...
		for ( ; top < n; ++top )
			mulb(fact+top+1, fact+top, top+1);
		printf("%d\n", digsum( fact+n ));
	}
